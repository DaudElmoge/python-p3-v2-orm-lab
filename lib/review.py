...
class Review:
    ...
    def save(self):
        """Insert the Review instance into the database and store it in `all`."""
        sql = """
    ...
    def find_by_id(cls, id):
        """Return a Review instance matching the given ID."""
        sql = "SELECT * FROM reviews WHERE id = ?"
        row = CURSOR.execute(sql, (id,)).fetchone()
        return cls.instance_from_db(row) if row else None

    def update(self):
        """Update the database with the current Review's attributes."""
        sql = """
            UPDATE reviews
    ...
    def delete(self):
        """Delete the Review from the database and `all` dictionary."""
